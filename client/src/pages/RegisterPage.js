...
const RegisterPage = () => {
    ...
    const submitHandler = async (e) => {
        try{
            e.preventDefault();
            if(username.length < 3) throw new Error("회원ID가 너무 짧습니다. 3자 이상으로 해주세요.");
            if(password.length < 6) throw new Error("비밀번호가 너무 짧습니다. 6자 이상으로 해주세요.");
    ...
    return (
        <div style={{
            ...
            <form onSubmit={submitHandler}>
                ...
                <CustomInput label="password check" value={passwordCheck} type="password" setValue={setPasswordCheck}/>
